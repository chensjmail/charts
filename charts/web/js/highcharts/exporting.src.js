/**
 * @license Highcharts JS v4.1.4 (2015-03-10) Exporting module (c) 2010-2014
 *          Torstein Honsi License: www.highcharts.com/license
 */
...
(function(Highcharts){ // encapsulate
	...
	extend(Chart.prototype, {
		...
		/**
		 * A collection of regex fixes on the produces SVG to account for
		 * expando properties, browser bugs, VML problems and other. Returns a
		 * cleaned SVG.
		 */
		sanitizeSVG:function(svg){
			return svg.replace(/zIndex="[^"]+"/g, '').replace(/isShadow="[^"]+"/g, '').replace(/symbolName="[^"]+"/g, '').replace(/jQuery[0-9]+="[^"]+"/g, '').replace(/url\([^#]+#/g, 'url(#').replace(/<svg /, '<svg xmlns:xlink="http://www.w3.org/1999/xlink" ').replace(/ (NS[0-9]+\:)?href=/g, ' xlink:href=') // #3567
			.replace(/\n/, ' ')
			...
			/*
			 * This fails in IE < 8 .replace(/([0-9]+)\.([0-9]+)/g, function(s1,
			 * s2, s3) { // round off to save weight return s2 +'.'+ s3[0]; })
			 */
			...
			// IE specific
			.replace(/<IMG /g, '<image ').replace(/height=([^" ]+)/g, 'height="$1"').replace(/width=([^" ]+)/g, 'width="$1"').replace(/hc-svg-href="([^"]+)">/g, 'xlink:href="$1"/>').replace(/id=([^" >]+)/g, 'id="$1"').replace(/class=([^" >]+)/g, 'class="$1"').replace(/ transform /g, ' ').replace(/:(path|rect)/g, '$1').replace(/style="([^"]+)"/g, function(s){
		...
		getSVG:function(additionalOptions){
			var chart= this, chartCopy, sandbox, svg, seriesOptions, sourceWidth, sourceHeight, cssWidth, cssHeight, options= merge(chart.options, additionalOptions); // copy
																																										// the
																																										// options
																																										// and
																																										// add
			...
			if(!doc.createElementNS){
				/* jslint unparam: true *//*
										 * allow unused parameter ns in function
										 * below
										 */
			...
			each(chart.series, function(serie){
				...
				if(!seriesOptions.isInternal){ // used for the navigator series
												// that has its own option set
					options.series.push(seriesOptions);
				}
			});

			// Axis options must be merged in one by one, since it may be an
			// array or an object (#2022, #3900)
		...
		exportChart:function(options, chartOptions){
			...
			Highcharts.post(options.url, {
				...
				type:options.type,
				width:options.width || 0, // IE8 fails to post undefined
											// correctly, so use 0
		...
		contextMenu:function(className, items, x, y, width, height, button){
			var chart= this, navOptions= chart.options.navigation, menuItemStyle= navOptions.menuItemStyle, chartWidth= chart.chartWidth, chartHeight= chart.chartHeight, cacheName= 'cache-' + className, menu= chart[cacheName], menuPadding= mathMax(width, height), // for
																																																																		// mouse
																																																																		// leave
																																																																		// detection
			...
			if(!menu){
				...
				each(items, function(item){
					if(item){
						...
						// Keep references to menu divs to be able to destroy
						// them
						chart.exportDivElements.push(element);
					}
				});

				// Keep references to menu and innerMenu div to be able to
				// destroy them
